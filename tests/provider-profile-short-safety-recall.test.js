...
describe("short provider contract safety and recall", () => {
  ...
  it("checks a contact-bearing exact-NPI page before returning specialty only", () => {
    ...
    expect(providerProfileSystemInstructions).toMatch(/Before answering, emit every eligible supported field/i);
  ...
  it("resolves alternate-NPI, same-name geography, and phone-purpose conflicts", () => {
    expect(providerProfileSystemInstructions).toMatch(/same-name alternate NPI.*open the best conflict source before emitting the implicated fact/i);
