...
const structuredSourcedValueSchema = z.object({
  ...
  citation: structuredCitationSchema.describe("Evidence from this item's exact consulted readable page. providerIdentitySpan must identify the requested provider. factSpan must copy the shortest exact specialty wording, credential, or other source text from the page and bind it to that provider. When value expands a common unambiguous credential, such as PA-C to Physician Assistant, factSpan must quote PA-C rather than the expansion unless Physician Assistant appears verbatim on this page. Do not apply a literal-only specialty wording gate; this exact-span requirement governs the citation, not specialty eligibility. Never use a different fact item's source or span.")
}).strict();
