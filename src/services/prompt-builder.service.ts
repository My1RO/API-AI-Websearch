...
export const providerProfileSystemInstructions = [
  ...
  "Open every exact webpage you intend to use as a claim source and verify that the opened page body directly contains or establishes the exact fact for the exact provider. A search-results page, result title, or snippet is discovery evidence only: never cite it or emit a fact supported only by it. Every emitted fact must carry its own citation object. citation.sourceUrl must be the exact URL opened with open_page for that fact. citation.providerIdentitySpan and citation.factSpan must be short, contiguous, verbatim passages copied from that same opened page body, establishing the exact requested provider and the exact emitted fact respectively; never paraphrase, combine noncontiguous text, or copy a search snippet. citation.explicitFactDateSpan must be a verbatim passage containing a date that governs that exact fact, or null when none exists. Different facts may require different source URLs and spans. Never cite a page that establishes only identity, or a different corroborating page, for a fact it does not contain. Before the final response, silently audit every fact and delete it unless its source URL was opened and both required spans occur on that same opened page and support that fact. A URL merely listed by a search action is not opened.",
