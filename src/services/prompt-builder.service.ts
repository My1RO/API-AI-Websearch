...
export const providerProfileSystemInstructions = [
  ...
  "Search the exact NPI with the quoted name first. Broaden only as needed to the NPI, legal/DBA/alias name, location hints, or a likely first-party page. NPI plus name are primary identity; providerId is correlation data. City/state/ZIP may be stale search hints, and specialty is only a weak cross-check. For every provider, run at least one exact-name/location or likely-organization first-party search and inspect the best plausible result before answering; do not stop at registry evidence. Search results, titles, snippets, URLs, and error or access-challenge pages are discovery leads, not readable evidence.",
  ...
  "For organizations, inspect relevant surfaced same-name alternate-NPI evidence before using a first-party operational bundle. A conflict is relevant only when readable evidence assigns the candidate value or exact domain/page bundle to another NPI, organizational subpart, or incompatible operation; similar names, proximity, branding, affiliation, or a location mismatch alone are insufficient. Omit the implicated value unless separate affirmative evidence attaches that same value to the requested NPI or establishes shared/concurrent use. For Entity Type 2 organizations at the same base street, treat a conflicting suite or subpart as unresolved unless such evidence resolves it; omit only that component and preserve an independently exact-NPI base address. Apply conflicts field-locally: an address or phone conflict does not itself implicate a domain. An implicated domain requires explicit requested-NPI attachment, shared use, or rebranding/acquisition/ownership-continuity evidence. For ordinary unimplicated facts, do not demand exclusivity.",
  ...
  "Every emitted fact needs its own citation to one consulted readable page. After values are fixed, make one citation-only pass over already-consulted exact-provider pages supporting that same complete value or a faithful formatting equivalent: use first-party evidence when available, otherwise government, then an eligible directory. Replace a lower-tier citation whenever a readable higher-tier page supports the same value; this never changes the selected value. sourceUrl is that exact fact-supporting page URL, never a search page, snippet, metadata/tool-action URL, unread/error page, or different corroborating page. providerIdentitySpan and factSpan are the shortest sufficient contiguous quotations from that same page; do not use ellipses, join passages, paraphrase, invent labels, or rewrite source tokens to match normalization. Identity normally needs exact NPI or full requested name plus compatible organization/location; a shortened name is acceptable only when the whole provider-specific page supplies multiple compatible biographical identifiers and no conflict. factSpan must support the complete fact and preserve rendered words/token order; markup, whitespace, and Unicode typography may be normalized without changing words. explicitFactDateSpan must co-bind the exact fact to an effective/current date; otherwise null. Retrieval dates, copyright years, generic page dates, and record-wide registry dates do not qualify.",
  "A website value is the exact URL of a consulted readable provider- or organization-specific first-party page and must equal citation.sourceUrl. The URL itself is its direct evidence; providerIdentitySpan and factSpan may reuse one exact page-local provider/organization heading. Emit the website only when both required spans can be copied exactly from that page; omit it rather than fabricate or combine text. A biography/team page is eligible without NPI or contact parity when it gives the unambiguous full name plus compatible credential/specialty and practice/location. Multiple compatible affiliations do not disqualify it. Omit an unresolved domain bound to another NPI/operation.",
