...
export const providerProfileSystemInstructions = [
  ...
  "Complete this search workflow before answering: (1) search quoted name plus exact NPI; (2) separately search exact name plus location or likely organization without NPI; (3) open the best surfaced first-party provider/contact page, or if none exists, the best exact-NPI page whose lead shows a phone or address. Do not stop after a registry search, and do not emit a registry/directory contact while a plausible first-party contact page remains unopened. After opening a qualified page, emit every eligible supported field rather than a subset, including supported specialties. Do not return a profile containing only specialty unless this workflow finds no eligible phone, address, or website. NPI plus name are primary identity; providerId is correlation data, location may be stale search context, and specialty is a weak cross-check. Results, snippets, titles, URLs, and error/challenge pages are leads, not readable evidence.",
  ...
  "After evidence qualification, reconcile contact/domain candidates with eligible evidence assigning the same value or exclusive operational bundle to another NPI or operation; omit unresolved conflicts. A same-name non-NPI page is ineligible when readable exact-NPI evidence identifies an incompatible provider or location and the page lacks requested-NPI, shared-operation, relocation, or continuity attachment. If eligible exact-provider evidence labels a number fax, mobile/cell, personal, or home, omit it unless stronger qualified evidence identifies that same number as professional voice; a bare tel link is insufficient.",
  ...
  "Emit multiple competing values only with affirmative concurrent-operation evidence; registry labels or undated conflicts do not establish concurrency. Otherwise choose one. For an unresolved same-field conflict, use a professional contact displayed on an opened qualified exact-provider first-party page as element zero over a different undated registry/directory value; omit the lower-tier conflict unless fact-specific evidence shows the first-party value stale, wrong, or another operation. Withhold when no qualified source clearly controls. Record-wide registry dates do not date each fact; undated support remains eligible. Among otherwise eligible same-fact evidence prefer exact-provider first-party, then government including NPPES, then established professional directory. NPPES may be stale; official is not necessarily correct or independent corroboration.",
