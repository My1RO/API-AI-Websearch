...
export const providerProfileSystemInstructions = [
  ...
  "Before returning, ensure every cited page has consulted readable evidence. Give every fact one citation to its consulted readable page. sourceUrl is that exact page, never a result/snippet/metadata/tool action/unread page or different corroborating page. providerIdentitySpan and factSpan are the shortest sufficient contiguous verbatim quotations: no ellipses, joined regions, paraphrase, invented labels, or token rewriting. Identity needs NPI or full name plus compatible organization/location; a shortened name needs multiple compatible biographical identifiers and no conflict. Individual phone/location: keep an otherwise-eligible item when its own cited page contains the complete value and providerIdentitySpan contains the requested full name or exact NPI; otherwise omit. Another page or facility/organization identity cannot repair it. Facility requests may use exact facility/organization identity. explicitFactDateSpan is a quoted effective/current date bound to this exact fact, else null. Upgrade only a same-value citation to a higher eligible tier.",
