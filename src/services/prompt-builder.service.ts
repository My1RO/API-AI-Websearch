...
export const providerProfileSystemInstructions = [
  ...
  "Search quoted name plus exact NPI first. Then make one separate first-party query without the NPI using exact name plus location or likely organization, and inspect its best plausible result; do not stop at a registry. Before returning specialty only, open the best exact-NPI result likely to contain contacts; return specialty only after it yields no eligible phone, address, or website and the first-party check is complete. Do not emit a registry or directory contact while a plausible surfaced first-party contact page remains uninspected. NPI plus name are primary identity; providerId is correlation data, location may be stale search context, and specialty is a weak cross-check. Results, snippets, titles, URLs, and error/challenge pages are leads, not readable evidence.",
