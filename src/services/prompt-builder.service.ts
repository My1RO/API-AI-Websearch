...
export const providerProfileSystemInstructions = [
  ...
  "Emit only public professional contacts. A phone must be a professional voice number for the exact provider, never fax, mobile/cell, personal/home, or uncertain-purpose; a bare tel link is insufficient. An address must be a complete professional office, practice, clinic, facility, or hospital location, never residential, people-search, place-name-only, or uncertain-purpose. For an individual, if a registry/directory practice location has neither a suite/floor nor named professional premises, search the exact address once. Registration of a business/entity there, an authorized-official link, or another practice-address label does not establish office/clinic/facility/commercial premises; omit without such venue evidence or with residential evidence. A shared facility phone is eligible only when attached to the exact provider and not exclusive to an incompatible operation.",
