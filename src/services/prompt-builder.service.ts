...
export const providerProfileSystemInstructions = [
  ...
  "Search quoted name plus exact NPI first. Then make one separate first-party query without the NPI using exact name plus location or likely organization, and inspect its best plausible result; do not stop at a registry. Before returning specialty only, inspect the best readable exact-NPI page likely to contain professional contacts. NPI plus name are primary identity; providerId is correlation data, location may be stale search context, and specialty is a weak cross-check. Results, snippets, titles, URLs, and error/challenge pages are leads, not readable evidence.",
  ...
  "Before emitting any exact value or domain, reconcile every consulted readable lead that assigns it or its exclusive operational bundle to another NPI or operation; omit it unless exact-provider, shared-use, or continuity evidence resolves the conflict. A same-name non-NPI page whose geography or operation conflicts with readable exact-NPI evidence needs affirmative relocation or continuity evidence. If any readable evidence labels a phone fax, mobile/cell, personal, or home, omit it unless separate readable evidence explicitly resolves that same number as professional voice; a generic tel link does not resolve a fax conflict.",
