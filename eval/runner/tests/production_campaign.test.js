...
const { assertSdkTimeoutHeader, buildSchedule, deepFreeze, readCases, requestHeaders,
  serializeError } = require("../core/run_production_campaign.js");
const { estimateAiResponseCost, extractAiResponseUsage } = require("../core/usage_accounting.js");
...
assert.equal(estimateAiResponseCost(usage, {
  inputUsdPerMillion: 1, cachedInputUsdPerMillion: 1, outputUsdPerMillion: 1,
  webSearchUsdPerThousand: 1, pricingVersion: "synthetic-rate-card"
}).estimated, true);
...
try {
  ...
  const runnerFile = path.join(__dirname, "../core/run_production_campaign.js");
  ...
  const admitted = JSON.parse(childProcess.execFileSync(process.execPath,
    [path.join(__dirname, "../core/run_production_campaign.js"), manifestFile, "validate"],
    { encoding: "utf8", env }));
