...
const atomic = require("../core/run_atomic_evaluator.js");
...
try {
  ...
  const sourceRunnerAuthority = { schemaVersion: 2, authority: "unified_provider_evaluator",
    ...
    { name: "compile_campaign.js", sha256: "5".repeat(64), byteLength: 300 },
  ...
  const resumeRunnerAuthority = { schemaVersion: 2, authority: "unified_provider_evaluator",
    ...
    { name: "compile_campaign.js", sha256: "6".repeat(64), byteLength: 320 },
    { name: "run_evaluator_campaign.js", sha256: "2".repeat(64), byteLength: 200 }
  ] };
...
assert.equal(atomic.classifyEvaluatorInsufficiency(contentFilterFixture).reason, "EVALUATOR_CONTENT_FILTER");
...
try {
  ...
  const enospcFixture = JSON.parse(fs.readFileSync(path.join(__dirname,
    "fixtures/atomic_resume_artifact_loss.json"), "utf8"));
  const enospcRoot = path.join(resumeArtifactRoot, "synthetic-enospc");
  ...
  const timeoutFixture = JSON.parse(fs.readFileSync(path.join(__dirname,
    "fixtures/atomic_resume_timeout.json"), "utf8"));
  const timeoutRoot = path.join(resumeArtifactRoot, "synthetic-timeout");
...
(async () => {
  ...
  try {
    ...
    const exhaustedClient = { responses: { create: async () => structuredClone(contentFilterFixture) } };
