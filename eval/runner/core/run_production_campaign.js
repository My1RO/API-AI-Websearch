...
// ownership, accounting, and artifact shape remain identical.
...
const { estimateAiResponseCost, extractAiResponseUsage } = require("./usage_accounting.js");
...
const validateManifest = (manifest, manifestFile) => {
  ...
  "nonTreatmentSurfaceSha256 must pin the non-treatment runtime surface.");
...
const main = async () => {
  ...
  if (!process.env.AZURE_OPENAI_API_KEY) {
    const keyFile = process.env.AZURE_OPENAI_API_KEY_FILE;
    assert(keyFile && fs.existsSync(keyFile),
      "Set AZURE_OPENAI_API_KEY or AZURE_OPENAI_API_KEY_FILE before running a paid campaign.");
    process.env.AZURE_OPENAI_API_KEY = fs.readFileSync(keyFile, "utf8").trim();
  }
  process.env.TS_NODE_PROJECT ||= path.join(config.arms[0].root, "tsconfig.json");
  process.env.NODE_PATH = [config.runtime.sdkModules, process.env.NODE_PATH]
    .filter(Boolean).join(path.delimiter);
  require("node:module").Module._initPaths();
  require(path.join(config.runtime.sdkModules, "ts-node/register/transpile-only"));
  ...
  const runCell = async ({ armId, caseId, withinCasePosition }, schedulePosition) => {
    ...
    const usage = sends.map((send) => {
      ...
      const extracted = extractAiResponseUsage(send.raw);
      return { ...extracted, ...estimateAiResponseCost(extracted, RATE_CARD), latencyMs: send.latencyMs };
    });
    const artifact = {
