...
const STATUS = "sealed_before_synthesis_execution";
...
const authorizationPaths = (workspace, { allowMissingRoot = true } = {}) => {
  const override = process.env.PROVIDER_EVALUATOR_AUTHORIZATION_ROOT;
  const root = path.resolve(override
    || path.join(workspace, "eval-private/authorization"));
  assertContainedPath(workspace, root, "evaluator authorization directory", { allowMissing: allowMissingRoot });
...
const expectedHoldoutContentSetSha256 = () => {
  const configured = process.env.PROVIDER_EVALUATOR_EXPECTED_HOLDOUT_CONTENT_SET_SHA256;
  if (!isSha256(configured)) {
    throw new Error("PROVIDER_EVALUATOR_EXPECTED_HOLDOUT_CONTENT_SET_SHA256 is required for holdout validation.");
  }
  return configured;
};
...
const validateFinalWinnerLock = ({ workspace, protocol, armLock, judgeLock }) => {
  ...
  const holdoutOverride = process.env.PROVIDER_EVALUATOR_HOLDOUT_SEAL;
  const canonicalHoldoutSeal = path.resolve(holdoutOverride
    || path.join(workspace, "eval-private/HOLDOUT_SEAL.json"));
...
const validateBoundProtocol = ({ workspace, protocol, requireSealed = true, verifyData = true }) => {
  ...
  if (armLock.status !== "sealed_before_paid_production"
...
module.exports = {
  STATUS, STAGES, RUNTIME, EVALUATOR, HOLDOUT_DATA_FILES,
