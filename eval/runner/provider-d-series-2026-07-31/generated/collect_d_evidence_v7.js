...
const { canonicalExactSourceUrl, assertFetchablePublicUrl } = require("./public_url_canonicalizer.js");

const workspace = process.env.EDE_SHARED_WORKSPACE || "/Users/kui/lucie/EDE";
const protocolConfigPath = process.env.PROVIDER_D_SERIES_PROTOCOL_CONFIG
  ? path.resolve(process.env.PROVIDER_D_SERIES_PROTOCOL_CONFIG) : null;
...
const synthesisProtocol = protocolConfig?.schemaVersion === 2
  ? require(path.join(__dirname, "../scripts/d_synthesis_protocol.js")) : null;
if (synthesisProtocol) synthesisProtocol.validateShape(protocolConfig);
const sourceHarnessRoot = path.join(workspace, "test-evidence/provider-final-experiment-2026-07-30");
const DEFAULT_NORMALIZER_MODULE = path.join(sourceHarnessRoot, "scripts/full_source_ingestion_v6.js");
const NORMALIZER_MODULE_PATH = path.resolve(process.env.PROVIDER_D_EVALUATOR_NORMALIZER_MODULE || DEFAULT_NORMALIZER_MODULE);
...
const LEGACY_SUPPORTED_RUNS = Object.freeze({
  "provider-d-series-pilot-13x3-v1": Object.freeze({ runMode: "provider-d-series-pilot-13x3", expectedRows: 39 }),
  "provider-d-series-60x3-v1": Object.freeze({ runMode: "provider-d-series-60x3", expectedRows: 180 })
});
...
const COLLECTOR_VERSION = "provider-d-evidence-collector-v7-fair-global-browser-paced";
...
const fetchSourceResult = async ({ requestedUrl, productionRoot, pool, fetchImpl, transportPolicy, originLimiter, globalTransferLimiter }) => {
  ...
  try {
    ...
    return {
      ...
      fetchPolicyVersion: "d-series-exact-url-browser-paced-v7-fair-global",
...
const verifyProductionSeal = ({ productionRoot, preregistration }) => {
  ...
  if (!fs.existsSync(sealPath)) throw new Error("Immutable production-input-seal.json is required before any D-series evidence collection.");
  ...
  if (seal.schemaVersion !== 1 || seal.status !== "sealed_before_d_series_evidence_collection"
    ...
    throw new Error("D-series production seal does not match the frozen production run.");
  }
...
const main = async () => {
  const productionRoot = path.resolve(process.env.PROVIDER_D_SERIES_INPUT_ROOT || "");
  if (!productionRoot || !fs.existsSync(path.join(productionRoot, "preregistration.json"))) throw new Error("Set PROVIDER_D_SERIES_INPUT_ROOT to a sealed D-series production root.");
  let launchProvenance = null;
  const v10Helper = require(path.join(__dirname, "../scripts/d_v10_replay.js"));
  const v10Requested = productionRoot === v10Helper.PATHS.replayRoot
    || protocolConfig?.experimentId === "d21-d28-development-full-v10-replay-002"
    || Boolean(process.env.PROVIDER_D_V10_LAUNCH_SEAL || process.env.PROVIDER_D_V10_LAUNCH_SEAL_SHA256);
  if (v10Requested) {
    const configuredLaunchSeal = path.resolve(process.env.PROVIDER_D_V10_LAUNCH_SEAL || "");
    const launch = v10Helper.verifyLaunchSeal();
    if (productionRoot !== v10Helper.PATHS.replayRoot
      || protocolConfigPath !== v10Helper.PATHS.replayProtocol
      || configuredLaunchSeal !== v10Helper.PATHS.launchSeal
      || process.env.PROVIDER_D_V10_LAUNCH_SEAL_SHA256 !== sha256(launch.bytes)) {
      throw new Error("Canonical V10 replay collection requires its exact protocol, root, and sealed launch boundary.");
    }
    launchProvenance = {
      v10EvidenceLaunchSealSha256: sha256(launch.bytes),
      v10CollectorLauncherSha256: sha256(fs.readFileSync(v10Helper.PATHS.collectorLauncher))
    };
  }
  ...
  const runConfig = SUPPORTED_RUNS[preregistration.protocol];
  if (preregistration.schemaVersion !== 2 || !runConfig || preregistration.runMode !== runConfig.runMode) throw new Error("Unsupported D-series production preregistration.");
  if (!Array.isArray(preregistration.schedule) || preregistration.schedule.length !== runConfig.expectedRows) throw new Error(`D-series schedule must contain ${runConfig.expectedRows} arm/case rows.`);
