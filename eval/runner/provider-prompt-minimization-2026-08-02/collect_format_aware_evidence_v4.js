...
const ROOT = path.join(WORKSPACE, "test-evidence/provider-prompt-minimization-2026-08-02");
const NORMALIZER = path.join(__dirname, "format_aware_source_normalizer.js");
process.env.PROVIDER_D_EVALUATOR_NORMALIZER_MODULE = NORMALIZER;
...
const DEFAULT_CAMPAIGNS = Object.freeze([
  { name: "full-predecessor", armId: "LONG", input: path.join(ROOT, "runs/development-full-v1-production") },
  { name: "short-successor", input: path.join(ROOT, "runs/successor-holdout-v2-production") },
  { name: "candidate-72", input: path.join(ROOT, "runs/candidate72-diagnostic-v1-production") }
]);
const DRY_RUN = process.env.DRY_RUN !== "0";
const OUTPUT_BASE = path.resolve(process.env.OUTPUT_BASE || path.join(ROOT, "runs"));
...
const campaigns = configuredCampaigns || (campaignArguments.length ? campaignArguments : DEFAULT_CAMPAIGNS);
if (!Array.isArray(campaigns) || !campaigns.length || campaigns.some((campaign) =>
  !campaign.name || !campaign.input || (campaign.armId != null && !campaign.armId))) {
...
const main = async () => {
  // This intentionally rejects the historical P-vs-H default mixture. A
  // comparison campaign must bind identical case IDs and request payloads.
...
module.exports = { DEFAULT_CAMPAIGNS, admitAllCellSources, assertSameBattery, bindSharedArtifactsToCampaign,
