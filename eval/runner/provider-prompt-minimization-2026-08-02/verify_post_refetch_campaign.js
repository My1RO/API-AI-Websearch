...
const EXPECTED_ARMS = Object.freeze(["D36", "SUCCESSOR", "CANDIDATE72"]);
...
const expectedCasesFromEnvironment = (environment = process.env) => {
  const casePrefix = environment.EXPECTED_CASE_PREFIX || "H";
...
const verifyPostRefetchCampaign = ({ packetRoot, expectedArms = EXPECTED_ARMS,
  expectedCases = EXPECTED_CASES, requiredEmptyCases = ["SUCCESSOR/H006", "SUCCESSOR/H027"],
  ...
  return { schemaVersion: 1, status: "POST_REFETCH_CAMPAIGN_READY", packetRoot: root,
    ...
    reconstructedEmptySuccessorCases: requiredEmptyCases.map((key) => key.split("/")[1]),
    requiredEmptyCases,
...
const main = () => {
  ...
  const expectedArms = process.env.EXPECTED_ARMS
    ? process.env.EXPECTED_ARMS.split(",").map((value) => value.trim()).filter(Boolean) : EXPECTED_ARMS;
...
module.exports = { EXPECTED_ARMS, EXPECTED_CASES, expectedCasesFromEnvironment,
