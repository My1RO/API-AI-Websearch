...
const clarifiedSchema = require("./categorical_judge_schema_v13_bounded_synthesis_axes.js")
...
try {
  fs.writeFileSync(path.join(manualRoot, "manual-review-required.json"), JSON.stringify({
    status: "EVALUATOR_MANUAL_REVIEW_REQUIRED", caseIds: ["H003"]
  }));
  fs.writeFileSync(path.join(manualRoot, "manual-review-required-runtime.json"), JSON.stringify({
    status: "EVALUATOR_MANUAL_REVIEW_REQUIRED", caseIds: ["H003", "H010"]
  }));
  ...
  assert.deepEqual([...synthesis.atomicManualCaseIds(manualRoot)].sort(), ["H003", "H010"]);
...
const websiteUnreadableFixture = JSON.parse(fs.readFileSync(path.join(__dirname,
  "fixtures/synthesis_website_unreadable_p031.json"), "utf8"));
assert.equal(websiteUnreadableFixture.provenance.caseId, "P031");
...
const realAxisFixture = JSON.parse(fs.readFileSync(path.join(__dirname,
  "fixtures/synthesis_whole_packet_axis_p029.json"), "utf8"));
assert.equal(realAxisFixture.provenance.armId, "SHORT1C");
const realTraceCompact = { ...compact, atomicClaimReadings: [{ claimIndex: 0,
  sourceReadings: [{ sourceIndex: 0, groups: realAxisFixture.atomicGroups }] }] };
assert.throws(() => synthesis.validateSynthesisOutput({
  ...output,
  claimAssessments: [{ ...output.claimAssessments[0], ...realAxisFixture.observedInvalidSynthesis }]
}, packet, realTraceCompact), /own-citation-only/);
...
(async () => {
  ...
  try {
    const item = { armId: "A", caseId: "H010", dir: directory, packet,
    ...
    const filteredClient = { responses: { create: async () => structuredClone(realShapedFilter) } };
