...
const { authorityDigest, evaluatorRunnerAuthority } = require("./evaluator_runner_authority.js");
...
const SCHEMA_FILE = path.join(process.env.EDE_SHARED_WORKSPACE || "/Users/kui/lucie/EDE",
...
try {
  ...
  const decisions = arms.map((arm) => {
    ...
    write(path.join(unitRoot, "review-output.json"), { startedAt: "2026-08-02T00:00:00.000Z",
      completedAt: "2026-08-02T00:00:00.400Z", durationMs: 400,
  ...
  childProcess.execFileSync(process.execPath, [path.join(__dirname, "compile_corrected_campaign.js"), manifestFile],
  ...
  assert.throws(() => childProcess.execFileSync(process.execPath,
    [path.join(__dirname, "compile_corrected_campaign.js"), manifestFile], {
      stdio: "pipe", env: { ...process.env, ALLOW_HISTORICAL_REPRODUCTION: "YES" }
    }));
